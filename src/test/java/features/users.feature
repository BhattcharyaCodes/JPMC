Feature: This feature is to check /users GET API
  ...
  Scenario: Retrieve a specific valid User by name
    Given a user exists with a valid name
    When I perform GET operation for for search by name  "/users/?name=Leanne%20Graham"
    Then I should see the user's name for search by name as "Leanne Graham"
